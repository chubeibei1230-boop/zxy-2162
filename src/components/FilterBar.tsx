import { Filter, X, ChevronDown, ChevronUp, ShieldAlert } from 'lucide-react';
import { useAppStore } from '@/store/useAppStore';
import { CATEGORY_LABELS, REVIEW_STATUS_LABELS, HANDOVER_STATUS_LABELS, BATCH_RISK_LABELS } from '@/types';
  ...
  return (
    <div className="bg-white rounded-xl shadow-card p-4 border border-navy-100">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-7 gap-3">
        ...
        <div>
          <label className="block text-xs text-navy-500 mb-1.5 flex items-center gap-1">
          ...
          >
            ...
            {Object.entries(BATCH_RISK_LABELS).map(([key, label]) => (
              <option key={key} value={key}>
                {label}
              </option>
            ))}
